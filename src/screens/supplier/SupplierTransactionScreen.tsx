...
import SupplierEditModal from '../../components/CustomEditModal';
...
const SupplierTransactionScreen = ({ route, navigation }: Props) => {
  ...
  const { transction_1, supplierData } = route.params;

  const [isAlertVisible, setAlertVisible] = React.useState(false);
  const [settlementAmount, setSettlementAmount] = useState(0);
  const [isEditSupplier, setIsEditSupplier] = useState(false)
  const [inputValue, setInputValue] = React.useState("")
  const [supplier, setSupplier] = React.useState<Supplier>(supplierData)
  ...
  const handleSMS = () => {
    if (!supplier?.name || settlementAmount == null) {
      Alert.alert("Error", "Supplier name or settlement amount is missing.");
      return;
    }
          ...
          <View style={styles.headerLeft}>
            ...
            <View style={styles.nameContainer}>
              ...
              <Text
                style={[styles.supplierLabel, { color: themeProperties.textColor, fontSize: themeProperties.textSize - 4 }]}
              >
                {supplier?.type === "supplier" ? t('supplier') : t('supplier')}
              </Text>
            </View>
          </View>
          {/* "View settings" link */}
